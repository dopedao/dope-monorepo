...
export default class Boot extends Scene {
  ...
  private _logNetworkEvents(networkHandler: NetworkHandler)
  {
    ...
    networkHandler.emitter.on(NetworkEvents.DISCONNECTED, () => this._log("Disconnected from server"));
    ...
    networkHandler.emitter.on(NetworkEvents.ERROR, (data: DataTypes[NetworkEvents.ERROR]) => console.warn(`Error ${data.code}: ${data.message}`));
    networkHandler.emitter.on(NetworkEvents.PLAYER_HANDSHAKE, (data: DataTypes[NetworkEvents.PLAYER_HANDSHAKE]) => this._log(`Got player handshake: ${data.id}`));
  ...
  private _logEvents()
  {
    ...
    EventHandler.emitter().on(Events.PLAYER_INVENTORY_ADD_ITEM, (item: Item, pickup?: boolean) => this._log(`Added item to inventory: ${item.name}
        picked up?: ${pickup ?? false}`));
    EventHandler.emitter().on(Events.PLAYER_INVENTORY_REMOVE_ITEM, (item: Item, drop?: boolean) => this._log(`Removed item from inventory: ${item.name}
        dropped?: ${drop ?? false}`));
    ...
    EventHandler.emitter().on(Events.PLAYER_CITIZEN_INTERACT, (citizen: Citizen) => this._log(`Started interaction with citizen: ${citizen.name}`));
    EventHandler.emitter().on(Events.PLAYER_CITIZEN_INTERACT_FINISH, (citizen: Citizen, cancelled: boolean) => this._log(`Finished interaction with citizen: ${citizen.name}
        cancelled?: ${cancelled}`));

    EventHandler.emitter().on(Events.PLAYER_QUEST_NEW, (quest: Quest) => this._log(`New quest: ${quest.name}`));
    EventHandler.emitter().on(Events.PLAYER_QUEST_START, (quest: Quest) => this._log(`Started quest: ${quest.name}`));
    EventHandler.emitter().on(Events.PLAYER_QUEST_COMPLETE, (quest: Quest) => this._log(`Completed quest: ${quest.name}`));

    EventHandler.emitter().on(Events.CHAT_MESSAGE, (hustler: Hustler, message: string) => this._log(`${hustler.name} says: ${message}`));
  }
