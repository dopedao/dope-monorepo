...
export default class EventHandler {
  ...
  constructor() {
    ...
    EventHandler._emitter.on(Events.PLAYER_INVENTORY_ADD_ITEM, (item: Item, pickup?: boolean) =>
      this._log(`Added item to inventory: ${item.name}
            picked up?: ${pickup ?? false}`),
    );
    EventHandler._emitter.on(Events.PLAYER_INVENTORY_REMOVE_ITEM, (item: Item, drop?: boolean) =>
    ...
    EventHandler._emitter.on(Events.PLAYER_CITIZEN_INTERACT, (citizen: Citizen) =>
      this._log(`Started interaction with citizen: ${citizen.name}`),
    );
    EventHandler._emitter.on(Events.PLAYER_CITIZEN_INTERACT_COMPLETE, (citizen: Citizen) =>
      this._log(`Completed interaction with citizen: ${citizen.name}`),
    );
    EventHandler._emitter.on(Events.PLAYER_CITIZEN_INTERACT_CANCEL, (citizen: Citizen) =>
      this._log(`Canceled interaction with citizen: ${citizen.name}`),
    );

    EventHandler._emitter.on(Events.PLAYER_QUEST_NEW, (quest: Quest) =>
      this._log(`New quest: ${quest.name}`),
    );
    EventHandler._emitter.on(Events.PLAYER_QUEST_START, (quest: Quest) =>
      this._log(`Started quest: ${quest.name}`),
    );
    EventHandler._emitter.on(Events.PLAYER_QUEST_COMPLETE, (quest: Quest) =>
